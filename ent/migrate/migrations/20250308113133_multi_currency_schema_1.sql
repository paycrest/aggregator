...
-- Create "fiat_currency_providers" table if it doesn't exist
CREATE TABLE IF NOT EXISTS "fiat_currency_providers" ("fiat_currency_id" uuid NOT NULL, "provider_profile_id" character varying NOT NULL, PRIMARY KEY ("fiat_currency_id", "provider_profile_id"), CONSTRAINT "fiat_currency_providers_fiat_currency_id" FOREIGN KEY ("fiat_currency_id") REFERENCES "fiat_currencies" ("id") ON UPDATE NO ACTION ON DELETE CASCADE, CONSTRAINT "fiat_currency_providers_provider_profile_id" FOREIGN KEY ("provider_profile_id") REFERENCES "provider_profiles" ("id") ON UPDATE NO ACTION ON DELETE CASCADE);
