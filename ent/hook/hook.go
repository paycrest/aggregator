...
// Mutate calls f(ctx, m).
func (f ProviderBalancesFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ProviderBalancesMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ProviderBalancesMutation", m)
}
...
// The ProviderOrderAssignmentFunc type is an adapter to allow the use of ordinary
// function as ProviderOrderAssignment mutator.
type ProviderOrderAssignmentFunc func(context.Context, *ent.ProviderOrderAssignmentMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ProviderOrderAssignmentFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ProviderOrderAssignmentMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ProviderOrderAssignmentMutation", m)
}
