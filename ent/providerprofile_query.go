...
func (ppq *ProviderProfileQuery) QueryAssignedOrders() *PaymentOrderQuery {
	query := (&PaymentOrderClient{config: ppq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := ppq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (ppq *ProviderProfileQuery) WithAssignedOrders(opts ...func(*PaymentOrderQuery)) *ProviderProfileQuery {
	query := (&PaymentOrderClient{config: ppq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (ppq *ProviderProfileQuery) loadAssignedOrders(ctx context.Context, query *PaymentOrderQuery, nodes []*ProviderProfile, init func(*ProviderProfile), assign func(*ProviderProfile, *PaymentOrder)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*ProviderProfile)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
