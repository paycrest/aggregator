...
func TestDecodeLocalTransferFeeSplitEvent(t *testing.T) {
	...
	sa := big.NewInt(1_000_000)
	pa := big.NewInt(2_000_000)
	ag := big.NewInt(3_000_000)
	data := make([]byte, 96)
...
func TestDecodeSenderFeeTransferredEvent(t *testing.T) {
	orderID := common.HexToHash("0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
	sender := common.HexToAddress("0x1234567890123456789012345678901234567890")
	amount := big.NewInt(5_000_000)

	log := types.Log{
		Topics: []common.Hash{
			common.HexToHash(SenderFeeTransferredEventSignatureLegacy),
			orderID,
			common.BytesToHash(common.LeftPadBytes(sender.Bytes(), 32)),
			common.BytesToHash(common.LeftPadBytes(amount.Bytes(), 32)),
		},
		Data: nil,
	}

	decoded, err := DecodeSenderFeeTransferredEvent(log)
	require.NoError(t, err)
	idx := decoded["indexed_params"].(map[string]interface{})
	nidx := decoded["non_indexed_params"].(map[string]interface{})
	assert.Equal(t, orderID.Hex(), idx["orderId"])
	assert.Equal(t, sender.Hex(), idx["sender"])
	assert.Equal(t, "5000000", idx["amount"])
	assert.Empty(t, nidx)
}
