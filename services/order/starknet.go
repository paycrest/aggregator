...
func (s *OrderStarknet) CreateOrder(ctx context.Context, orderID uuid.UUID) error {
	...
	if order.Edges.ReceiveAddress == nil && order.Edges.LinkedAddress == nil {
		return fmt.Errorf("%s - CreateOrder.missingSaltSource", orderIDPrefix)
	}
...
func (s *OrderStarknet) RefundOrder(ctx context.Context, network *ent.Network, orderID string) error {
	...
	feeFelt := new(felt.Felt).SetBigInt(u.ToSubunit(decimal.NewFromInt(0), lockOrder.Edges.Token.Decimals))

	gatewayContractAddressFelt, err := utils.HexToFelt(lockOrder.Edges.Token.Edges.Network.GatewayContractAddress)
	if err != nil {
		return fmt.Errorf("%s - RefundOrder.parseGatewayAddress: %w", orderIDPrefix, err)
	}
	gatewayOrderIDFelt, err := utils.HexToFelt(lockOrder.GatewayID)
	if err != nil {
