...
func (s *PriorityQueueService) AssignPaymentOrder(ctx context.Context, order types.PaymentOrderFields) error {
	...
	if err == nil {
		...
		if currentOrder.Status != paymentorder.StatusPending && currentOrder.Status != paymentorder.StatusDeposited {
			logger.WithFields(logger.Fields{
				"OrderID": order.ID.String(),
				"Status":  currentOrder.Status,
			}).Warnf("AssignPaymentOrder: Order is not in pending/deposited state, skipping assignment")
