...
func (s *PriorityQueueService) AssignPaymentOrder(ctx context.Context, order types.PaymentOrderFields) error {
	...
	currency, err := resolveOrderFieldsCurrency(ctx, order)
	if err != nil {
		return err
	}
	if order.Token == nil {
	...
	requiredBalance := order.Amount
	if rateSide == RateSideSell {
		requiredBalance = order.Amount.Mul(order.Rate).RoundBank(0)
	}
	...
	candidates, err := storage.Client.ProviderOrderToken.Query().
		Where(
			...
			providerordertoken.HasProviderWith(
				providerprofile.IDNEQ(orderConf.FallbackProviderID),
				providerprofile.IsActive(true),
				providerprofile.VisibilityModeEQ(providerprofile.VisibilityModePublic),
			),
		).
	...
	for _, candidate := range candidates {
		...
		hasBalance := false
		if rateSide == RateSideBuy {
			hasBalance, err = storage.Client.ProviderBalances.Query().
				Where(
					providerbalances.HasProviderWith(providerprofile.IDEQ(providerID)),
					providerbalances.HasTokenWith(token.IDEQ(order.Token.ID)),
					providerbalances.IsAvailableEQ(true),
					providerbalances.AvailableBalanceGTE(requiredBalance),
				).
				Exist(ctx)
		} else {
			hasBalance, err = storage.Client.ProviderBalances.Query().
...
func (s *PriorityQueueService) TryFallbackAssignment(ctx context.Context, order *ent.PaymentOrder) error {
	...
	bucketCurrency, err := resolveOrderCurrency(ctx, currentOrder)
	if err != nil {
		return fmt.Errorf("fallback: resolve order currency: %w", err)
	}
...
func (s *PriorityQueueService) sendOrderRequest(ctx context.Context, order types.PaymentOrderFields) error {
	...
	currencyEnt, err := resolveOrderFieldsCurrency(ctx, order)
	if err != nil {
		return err
	}
